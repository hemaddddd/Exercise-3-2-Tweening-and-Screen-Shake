...
func _on_Ball_body_entered(body):
	...
	$Umages/Highlight.scale = Vector2(2,2)
	tween.tween_property($Images/Highlight, "scale", Vector2(1,1), time_highlight_size).set_trans(Tween.TRANS_BOUNCE).set_ease_(Tween.EASE_IN)
	wobble_direction = linear_velocity.orthogonal().orthogonal().normalized()
